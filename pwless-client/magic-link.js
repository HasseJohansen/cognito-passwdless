...
export const requestSignInLink = ({ usernameOrAlias, currentStatus, statusCb, }) => {
...
async function authenticateWithSignInLink({ usernameOrAlias, fragmentIdentifier, currentStatus, clientMetadata, session, abort, }) {
    const { clientId, storage, debug } = configure();
    if (currentStatus && busyState.includes(currentStatus)) {
        throw new Error(`Can't authenticate with link while in status ${currentStatus}`);
    }
    session ?? (session = (await storage.getItem(`Passwordless.${clientId}.${usernameOrAlias}.session`)) ?? undefined);
    await storage.removeItem(`Passwordless.${clientId}.${usernameOrAlias}.session`);
    if (!session) {
