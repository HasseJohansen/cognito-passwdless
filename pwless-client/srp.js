...
async function getConstants() {
    if (!_CONSTANTS) {
        ...
        const k = arrayBufferToBigInt(await window.crypto.subtle.digest("SHA-256", hexToArrayBuffer(`${padHex(N.toString(16))}${padHex(g.toString(16))}`)));
        _CONSTANTS = {
            g,
            N,
            k,
...
async function calculateSrpSignature({ smallA, largeAHex, srpBHex, salt, userPoolId, username, password, secretBlock, }) {
    ...
    const u = await window.crypto.subtle.digest("SHA-256", hexToArrayBuffer(aPlusBHex));
    const [, userPoolName] = userPoolId.split("_");
    const usernamePasswordHash = await window.crypto.subtle.digest("SHA-256", new TextEncoder().encode(`${userPoolName}${username}:${password}`));
    const x = await window.crypto.subtle.digest("SHA-256", await new Blob([
    ...
    const prkKey = await window.crypto.subtle.importKey("raw", hexToArrayBuffer(saltHkdfHex), {
        name: "HMAC",
        hash: { name: "SHA-256" },
    }, false, ["sign"]);
    const prk = await window.crypto.subtle.sign("HMAC", prkKey, hexToArrayBuffer(ikmHex));
    const hkdfKey = await window.crypto.subtle.importKey("raw", prk, {
        name: "HMAC",
        hash: { name: "SHA-256" },
    }, false, ["sign"]);
    const hkdf = (await window.crypto.subtle.sign("HMAC", hkdfKey, infoBits)).slice(0, 16);
    ...
    const signatureKey = await window.crypto.subtle.importKey("raw", hkdf, {
        name: "HMAC",
        hash: { name: "SHA-256" },
    }, false, ["sign"]);
    const signatureString = await window.crypto.subtle.sign("HMAC", signatureKey, msg);
    return {
        timestamp,
